  ...
  if (url) {
    ...
    Meteor.call('getEmbedlyData', url, function (error, data) {
      ...
      if (data) {
        ...
        // generate hash for screenshotmachine api request
        var screenshotSrc = function() {
          url = data.url;
          secret = Meteor.settings.public.screenshotMachineSecret;
          var hash = CryptoJS.MD5(url + secret).toString();
          // change button text if image source is updated
          $('.filepicker').text('Replace Image');
          return 'https://api.screenshotmachine.com/?key=0151a3&size=T&hash=' + hash + '&url=' + url;
        }
        // set source of image
        $('.image-thumbnail').attr('src', screenshotSrc());
